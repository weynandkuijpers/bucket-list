## Bukket list

This is my bucket list.  I am sure this list will grow for the coming months / years but I will start executing towards this list latest in 2030, this is the year that I will turn 60, or when Skipr exits..  For now I will keep this list to myself, at some point in time I will start to publish it to my family.


The point of this list is to set goals  and focus for my life.

### Family
- [ ] Maledives Holiday
- [ ] Fiji holliday


### Bo
-  [ ] Son - Father trip to Japan
- [ ] Son - Father trip to the United States
- [ ] buy a house in NL for Bo and make sure all future maintenance, taxes and costs are covered
- [ ] buy a house in .... for Bo

### Vinn
- [ ] race training (circuit)
- [ ] buy a car for Vinn 
- [ ] buy a house in UAE for Vinn and make sure all maintenance, taxes and costs are covered
- [ ] buy a house in NL for Vinn

### Lucia
- [ ] Around the world to → London → New York → Las Vegas → Los Angelos → Tokyo → Macau → UAE (ras al Kheim?
) → Monaco   Round The World ticket (RTW).
- [ ] Cartier Nail necklace + Bracelet
- buy a house for Lucia in ..... and make sure all maintenance, taxes and costs are covered

### Friends
- [ ] Skipr ski trip in Japan (Andreas)

### Individual

- [ ] Build my own design mono loudspeaker and valve amplifier
- Purchase an old motorcycle and restore it to a working condition.  Requirements:
    - least amoount of electronics
    - carburator operated
    - 1 or 2 cyclinders
    - medium horsewpower  <= 100HP
- [ ] Ride this motorcycle (and while restoring there might be a "better" one in driving condition to start doing this) the
    - [ ] Stelvio pass in Italy
    - [ ] to Inverness in Scotland
    - [ ] to Mekka in the middle east
    - [ ] to the artic circle in Norway
    - [ ] East to West coast in the US
- [ ] Vanlife -> not a holliday, but a good year of continuous vanlife in Europe. Or East to West coast in the US
- [ ] 40 ft sailing life in / around the European seas
- [ ] Get a large tattoo
- [ ] Go up half dome, if possible using the cables route
